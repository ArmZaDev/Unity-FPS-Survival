...
public class EnemyController : MonoBehaviour {
	...
	void Chase() 
	{
		// enable the agent to move agian
		navAgent.isStopped = false;
		navAgent.speed = run_Speed;

		// set the player's position as the desrtination
		//because we rae chasing (running towards) the player
		...
		// if the distance betweet enemy and player is less then attack distance
		...
		else if (Vector3.Distance(transform.position, target.position) > chase_Distance)
        {
			...
			// reset the patrol timer so that the funtion
